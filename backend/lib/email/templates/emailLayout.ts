/**
 * Shared AYNAM email layout — table-based, inline-styled, static HTML.
 *
 * Email clients are not browsers: no flexbox/grid/JS/external CSS.
 * Everything is tables + inline styles, with a small <style> block only
 * for mobile media-query refinements (Gmail/Apple Mail honour these,
 * and the inline base styles keep Outlook presentable).
 *
 * Brand: bg #F5F5F2 · card #FFFFFF · ink #0A0A0A · muted #666666 ·
 * hairline rgba(0,0,0,0.12) · ~640px centred · generous whitespace.
 */
...
/** escaped text with user line breaks preserved as <br> */
...
export function detailRows(rows: DetailRow[]): string {
  return rows
    .map(
      (r) => `
      <tr>
        <td style="padding:14px 0 6px; border-top:1px solid rgba(0,0,0,0.08); font-family:${FONT}; font-size:11px; letter-spacing:1.6px; text-transform:uppercase; color:#666666; vertical-align:top;">
          ${esc(r.label)}
        </td>
      </tr>
      <tr>
...
export function outlineButton(href: string, label: string): string {
  return `
    <table role="presentation" cellpadding="0" cellspacing="0" align="center" style="margin:0 auto;">
      <tr>
        <td style="border:1px solid #0A0A0A; padding:12px 30px;">
          <a href="${esc(href)}" style="font-family:${FONT}; font-size:11px; letter-spacing:2px; text-transform:uppercase; color:#0A0A0A; text-decoration:none;">${esc(label)}</a>
        </td>
      </tr>
    </table>`;
}
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" align="left">
      <tr>
        <td style="background:#0A0A0A; padding:13px 32px;">
          <a href="${esc(href)}" style="font-family:${FONT}; font-size:11px; letter-spacing:2px; text-transform:uppercase; color:#FFFFFF; text-decoration:none;">${esc(label)}</a>
        </td>
      </tr>
    </table>`;
}
...
export type LayoutOptions = {
  /** absolute HTTPS banner image rendered edge-to-edge at the card top */
  bannerUrl?: string;
  preheader: string;
  eyebrow: string;
  titleLines: string[];
...
export function emailLayout(o: LayoutOptions): string {
  const banner = o.bannerUrl
    ? `
        <tr>
          <td style="background:#000000; font-size:0; line-height:0;" bgcolor="#000000">
    ...
    : `
        <tr>
          <td style="background:#000000; padding:34px 40px;" bgcolor="#000000">
            <span style="font-family:${FONT}; font-size:18px; font-weight:700; letter-spacing:6px; color:#FFFFFF;">AYNAM</span>
            <br />
    ...
    <tr>
      <td align="center" style="padding:32px 12px;">
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" class="aynam-shell" width="640" style="width:640px; max-width:640px; margin:0 auto;">
          <tr>
            <td style="background:#FFFFFF; border:1px solid rgba(0,0,0,0.12);">
              <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
                ${banner}
                <tr>
                  <td class="aynam-pad" style="padding:44px 48px 8px;">
                    <span style="font-family:${FONT}; font-size:11px; letter-spacing:2.4px; text-transform:uppercase; color:#666666;">${esc(o.eyebrow)}</span>
                    ...
                    ${
                      o.support
                        ? `<p style="margin:18px 0 0; font-family:${FONT}; font-size:15px; line-height:24px; color:#666666;">${esc(o.support)}</p>`
                        : ""
                    }
                  </td>
                </tr>
          ...
          <tr>
            <td align="center" style="padding:28px 20px 8px;">
              ...
              <br />
              <span style="font-family:${FONT}; font-size:11px; line-height:18px; color:#666666;">Independent software studio.<br />Software for a smarter tomorrow.</span>
              <br />
              <span style="font-family:${FONT}; font-size:11px; line-height:18px; color:#999999;">© 2026 AYNAM. All rights reserved.<br /><a href="${esc(o.siteUrl || "https://aynam.studio")}" style="color:#666666; text-decoration:underline;">${esc((o.siteUrl || "https://aynam.studio").replace(/^https?:\/\//, ""))}</a></span>
            </td>
          </tr>
        </table>
...
export const sectionLabel = (text: string): string =>
  `<p style="margin:28px 0 4px; font-family:${FONT}; font-size:11px; letter-spacing:2.4px; text-transform:uppercase; color:#666666;">${esc(text)}</p>`;
